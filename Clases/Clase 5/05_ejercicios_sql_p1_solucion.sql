...
-- Un CTE (Common Table Expression) es una tabla temporal definida con WITH
WITH asistencia_calc AS (
    SELECT cod_estudiante,
           nombre,
           (CASE WHEN a_19950501='X' THEN 1 ELSE 0 END +
            CASE WHEN a_19950502='X' THEN 1 ELSE 0 END +
...
SELECT cod_estudiante,
       nombre,
       dias_asistidos,
       ROUND(dias_asistidos / 23.0, 3)      AS tasa_asistencia,
       dias_asistidos / 23.0 > 0.80          AS asistencia_ok
...
SELECT CASE WHEN b."COD_ESTUDIANTE" IS NOT NULL THEN TRUE ELSE FALSE END AS nuevo_estudiante,
       COUNT(*) AS total
FROM   alumnos a
FULL JOIN nuevos_alumnos b
       ON a.cod_estudiante = b."COD_ESTUDIANTE"
...
-- Verificación: los cuatro caminos dan el mismo resultado
--
-- Usamos EXCEPT para verificar que no hay diferencias entre los resultados.
-- EXCEPT devuelve las filas que están en la primera consulta pero no en la segunda.
-- Si el resultado está vacío, las consultas son equivalentes.

-- Si esta consulta devuelve filas, los caminos A y B difieren
WITH asistencia_calc AS (
    SELECT cod_estudiante,
           (CASE WHEN a_19950501='X' THEN 1 ELSE 0 END +
            CASE WHEN a_19950502='X' THEN 1 ELSE 0 END +
...
nota_media AS (
    SELECT cod_estudiante, ROUND(AVG(nota), 3) AS nota_media
    FROM notas GROUP BY cod_estudiante
),
camino_a AS (
    SELECT v.cod_estudiante
    FROM viaje v
    INNER JOIN nota_media nm USING (cod_estudiante)
    INNER JOIN (SELECT cod_estudiante FROM asistencia_calc WHERE tasa_asistencia > 0.80) ac
           USING (cod_estudiante)
    WHERE v.permiso_firmado = TRUE
      AND v.cuotas_pagadas ~* '^[sacp]'
      AND nm.nota_media > 6.0
),
camino_b AS (
    ...
    WHERE v.permiso_firmado = TRUE
      ...
      AND nm.nota_media > 6.0
      AND NOT EXISTS (
          SELECT 1 FROM asistencia_calc ac
          WHERE ac.cod_estudiante = v.cod_estudiante
            AND ac.tasa_asistencia <= 0.80
      )
)
SELECT 'solo en A' AS diferencia, cod_estudiante FROM camino_a
EXCEPT
SELECT 'solo en A', cod_estudiante FROM camino_b
UNION ALL
SELECT 'solo en B', cod_estudiante FROM camino_b
EXCEPT
SELECT 'solo en B', cod_estudiante FROM camino_a;
...
SELECT nombre,
       ...
       ROUND(
           ...
           NULLIF(STDDEV(nota) OVER (PARTITION BY materia), 0)
       , 2) AS z_score
FROM   notas
WHERE  nota IS NOT NULL
...
SELECT nombre,
       ...
       DENSE_RANK()  OVER (PARTITION BY materia ORDER BY nota DESC) AS ranking
FROM   notas
WHERE  nota IS NOT NULL
